...
def test_stream_actions(stream_raw):
    ...
    for action_name in actions:
        ...
        assert args == (f'<turbo-stream action="{action_name}" target="id_of_object"><template>Hi</template></turbo-stream>',)
...
def test_stream_remove(stream_raw):
    ...
    assert stream_raw.call_count == 1
    args = stream_raw.mock_calls[0].args
    assert args == ('<turbo-stream action="remove" target="id_of_object"><template></template></turbo-stream>',)
